...
def diagonal(vents_map, x1, y1, x2, y2):
    d = y1
    if y1 > y2:
...
def hydrothermal_vents(vents):
    ...
    for i in range(len(vents)):
        if vents[i][0][0] == vents[i][1][0] or vents[i][0][1] == vents[i][1][1] or \
                abs(vents[i][0][0]-vents[i][1][0]) == abs(vents[i][0][1]-vents[i][1][1]):
    ...
    for i in range(len(first_vents)):
        ...
        elif first_vents[i][0][1] == first_vents[i][1][1]:
            vents_map = horizontally(vents_map, first_vents[i][1][1], first_vents[i][0][0], first_vents[i][1][0])

        else:
            if vents[i][0][0] < vents[i][1][0]:
                vents_map = diagonal(vents_map, vents[i][0][0], vents[i][0][1], vents[i][1][0], vents[i][1][1])
            else:
                vents_map = diagonal(vents_map, vents[i][1][0] , vents[i][1][1], vents[i][0][0], vents[i][0][1])
